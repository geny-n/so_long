...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bonus_check_valid_path.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ngeny <ngeny@student.42.fr>                +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/07 19:33:26 by ngeny             #+#    #+#             */
/*   Updated: 2024/06/03 19:03:06 by ngeny            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bonus_check_first(char **copy_map, int pos_x, int pos_y, int *flag)
{
	...
	bonus_check_first(copy_map, pos_x + 1, pos_y, flag);
	bonus_check_first(copy_map, pos_x - 1, pos_y, flag);
	bonus_check_first(copy_map, pos_x, pos_y + 1, flag);
	bonus_check_first(copy_map, pos_x, pos_y - 1, flag);
}
...
int	bonus_check_valid_path(t_data *data)
{
	...
	flag = 0;
	copy_map = bonus_duplicate_map(data);
	bonus_start_point (copy_map, data);
	bonus_check_first (copy_map, data->start_x, data->start_y, &flag);
	if (bonus_check_all_collect(copy_map, data, &flag) == 1)
	{
		bonus_free_copy_map(copy_map, data->map_h);
		return (1);
	}
	bonus_free_copy_map(copy_map, data->map_h);
	return (0);
